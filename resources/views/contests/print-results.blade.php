...
<head>
    ...
    <title>Print Results: {{ $contest->name }}</title>
    <style>
        * {
...
<body>
    ...
    <h2 style="margin-top:20px; margin-bottom:8px">Overall Rankings</h2>
    <table>
        <thead>
            <tr>
    ...
    <div class="page-break">
        ...
        <table>
            ...
            <tbody>
                ...
                <tr class="{{ $r['rank'] === 1 ? 'gold' : ($r['rank'] === 2 ? 'silver' : ($r['rank'] === 3 ? 'bronze' : '')) }}">
                    <td class="rank-cell">
                        @if($r['rank'] === 1) 🥇
                        @elseif($r['rank'] === 2) 🥈
                        @elseif($r['rank'] === 3) 🥉
                    ...
                    <td class="contestant-name">{{ $r['contestant']->getDisplayName() }}</td>
                    @foreach($exposure->criteria as $criteria)
                    <td>
                        {{ $r['criteria_scores'][$criteria->id]['average'] ?? '—' }}
                        <br><small>(w: {{ $r['criteria_scores'][$criteria->id]['weighted'] ?? '—' }})</small>
                    </td>
                    @endforeach
                    <td class="total-cell">{{ $r['total_weighted'] }}</td>
                </tr>
                @endforeach
            </tbody>
        </table>
