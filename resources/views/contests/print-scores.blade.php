...
<head>
    ...
    <title>Print Scores: {{ $contest->name }} — {{ $exposure->name }}</title>
    <style>
...
<body>
    ...
    @endif
    <h1>{{ $contest->name }}</h1>
    <h2>{{ $exposure->name }} — Score Sheet</h2>
    <p class="subtitle">{{ $contest->getTypeLabel() }} · {{ $contest->judges->count() }} Judges · {{ $contest->contestants->count() }} Contestants
        @if($exposure->is_final) · Final Round ({{ $exposure->carry_over_percentage }}% carry-over) @endif
    </p>
