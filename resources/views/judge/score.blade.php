...
@section('navbar')
<div class="navbar">
    <div class="brand">🧑‍⚖️ <span>Judge Scoring</span></div>
        ...
        @else
                ...
                @foreach($contestants as $contestant)
                    <div class="contestant-row" data-contestant-id="{{ $contestant->id }}">
                        <div class="flex items-center justify-between">
                            ...
                            <div class="final-score-display" style="text-align:center">
                        ...
                        {{-- Direct Final Score Input (Always Visible) --}}
            ...
            <script>
                ...
                toggle.addEventListener('change', function() {
                    // If toggle is disabled, don't allow changes
                    if (this.disabled) {
                        e.preventDefault();
                        return;
                    }
                    ...
                    document.querySelectorAll('[data-contestant-id]').forEach(row => {
                        ...
                        if (isFinalScoreOnly) {
                            ...
                            // Only set required if not readonly
                            ...
                            // Auto-populate final score input with current total if available
                ...
                form.addEventListener('submit', function(e) {
                    ...
                    if (isFinalScoreOnly) {
                        // In final score mode: check all direct final scores are filled
                        document.querySelectorAll('[data-contestant-id]').forEach(row => {
                            const directFinalScoreInput = row.querySelector('.direct-final-score');
                            // Skip validation if the input is hidden (already submitted)
                        ...
                        // In criteria mode: check all criteria are filled
                        document.querySelectorAll('[data-contestant-id]').forEach(row => {
                ...
                // Auto-calculate total when criteria scores change
                ...
                // Auto-detect and set toggle on page load based on existing scores
                window.addEventListener('load', function() {
                    const criteriaScoresWithValues = document.querySelectorAll('.criteria-score[value]').length;
                    const directFinalScoresWithValues = document.querySelectorAll('.direct-final-score[value]').length;
                    
                    // If there are direct final scores but no criteria scores, auto-check the toggle and disable it
                    if (directFinalScoresWithValues > 0 && criteriaScoresWithValues === 0) {
                        ...
                        // Make final score inputs read-only and update total display
                        document.querySelectorAll('[data-contestant-id]').forEach(row => {
                            const directFinalScoreInput = row.querySelector('.direct-final-score[value]');
                            const finalScoreDisplay = row.querySelector('.final-score-value');
                            
                            if (directFinalScoreInput) {
                                ...
                                // Update total display to show the final score value
                                if (finalScoreDisplay && directFinalScoreInput.value) {
                    ...
                    // If there are direct final scores, disable all criteria inputs as well
